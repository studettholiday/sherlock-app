...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT u.id, u.email, u.role, u.name, u.is_owner, u.school_id, s.name as school_name, s.api_key_encrypted, s.status as school_status, s.student_ai_enabled, s.student_downloads_enabled FROM users u JOIN schools s ON u.school_id = s.id WHERE u.id = $1',
      [decoded.userId]
    );
